...
class AJAX {
   ...
   */
  static #fetch (address, handler, options, errorHandler) {
    const prom = new Promise((resolve, reject) => {
      $("p.PHP-Exception")?.classList.remove("throw");
      errorHandler =
        errorHandler ??
        ((reas, txt) => {
          const exceptionCon = $("p.PHP-Exception");
      ...
      fetch(address, options).then((resp) => {
        ...
        if (resp.ok) {
          if (handler !== undefined) {
            ...
            if (
              options === undefined ||
              options.handlerType === undefined ||
              options.handlerType === "json"
            ) {
              resp
                .json()
                .then((json) =>
                  functionOrPromise(handler(json), resolve, reject)
                );
            }
          } else {
   ...
   * @param {(reason: any, txt: string)=>void=} errorHandler
   */
  static post (address, body, handler, options, errorHandler) {
